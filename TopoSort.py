...
class Graph(object):
    ...
    # Returns true if index has at least 1 incoming edge
    def hasIncomingEdge(self, index):
        for i in range(len(self.Vertices)):
            if self.adjMat[i][index]: return True
    ...
    def has_cycle(self):
        # Runs DFS on every vertex in graph and returns true if one of them is cyclic
        for i in range(len(self.Vertices)):
    ...
    def toposort(self):
        ...
        # Adds all the vertices without any incoming edges to topoList
        for i in range(len(self.Vertices)):
            if not self.hasIncomingEdge(i):
                topoList.append(self.Vertices[i].label)
                lastDegree.append(i)

        # Adds rest of the vertices in proper order
        while len(lastDegree) > 0:
            temp = []

            # Finds all edges that start with nodes in lastDegree list and adds to temp list
            for i in lastDegree:
                for j in range(len(self.Vertices)):
                    if self.adjMat[i][j]:
                        temp.append(self.Vertices[j].label)
            # Sorts temp into alphabetical order
            temp.sort()
            # Clears lastDegree list
            while len(lastDegree) > 0:
                lastDegree.pop()
            # Appends elements of temp list to toposort in proper order and appends to lastDegree for next iteration
            for i in temp:
                if i in topoList: topoList.remove(i)
                topoList.append(i)
                lastDegree.append(self.get_index(i))
